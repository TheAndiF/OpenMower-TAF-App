...
In **State0** wird die bisherige, überladene Kennzahlen-Zusammenfassung durch einen kompakten Freigabe-Banner ersetzt. Direkt darunter zeigt eine farbige Liste alle 12 Prüfpunkte: grün bedeutet erfüllt, gelb bedeutet prüfen/unklar und rot bedeutet blockiert. Wenn das Backend `blocking_stage` oder `blocking_key` liefert, markiert die App den betreffenden Prüfschritt zusätzlich als **blockierende Stufe**, damit sofort sichtbar ist, wo die Freigabe hängt.
